...
async function startServer() {
  ...
  app.post("/api/scheduled/daily-scan", async (req, res) => {
    try {
      ...
      console.log("[Heartbeat] Daily scan triggered by cron");
      const result = await runScan("cron");
      return res.json({ ok: true, scanRunId: result.scanRunId, results: result.results });
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      const stack = err instanceof Error ? err.stack : undefined;
      console.error("[Heartbeat] Daily scan error:", message);
